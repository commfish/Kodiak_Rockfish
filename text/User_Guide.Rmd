...
to load the *rockfish.RData* workspace located in the *code* sub directory of the current working directory and view a list of its contents, which currently consist of the three R functions `display_station_data()`, `get_haver_distance()` and `get_station_estimates()`.
...
which depends only on the fish's depth $d$. As for the star-based estimator, if the fish's distance $r$ from the center of the star pattern satisfies $r < \frac{w(d)}{2}$, the estimator will perfectly estimate station rockfish abundance ($\hat{N_S} \equiv 1$) for all possible star transects and thus have zero variance. In the opposite case the expected value of the estimator is
